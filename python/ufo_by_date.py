...
midi_output = 'midi_folder/ufo_by_date_small.mid'
...
def get_stats(reader, val_num):
    val_list = []
    for row in reader:
        if not '.' in row[val_num]:
            val_list.append(int(row[val_num]))
        else:
            val_list.append(round(float(row[val_num])))
